	  ...
	  if (length($s) ne length($para{'priseq'})){
		 ...
		 while ($position>=20){
			...
			while (($m<=24) and ($position >=$m)){
			   ...
			   #strand
			   $para{'strand'}='3';
			   print "calulating miRNA candidate: $para{'id'} at $id.";
			   parameter($serial,%para);#sent the information to calculate the parameters
			   $m +=1;
		   }
		   $serial +=1;
...
sub pmt{
   ...
   if (lc($class) eq "range"){
	  ...
	  @parameter=("prelength","mintcontent_c","mistart","mintcontent_u",
                    "unpairedrate_topstem","prentcontent_u","miinternalloop",
                    "length_upperstem","firstbase","pregc","internalloop_topstem",
                    "stability","internalloopnumber_topstem","internalloopnumber_lowerstem",
                    "miinternalloopnumber","prentcontent_c","preunpairedrate",
                    "length_topstem","prentcontent_a");#19
   }
