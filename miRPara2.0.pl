...
our $pri="";
our $version="2.0";
our $unafold=$ENV{'UNAFOLD'};
our $ct2out=$ENV{'CT2OUT'};
our $share=$ENV{'MIRPARA'};#the location of files
our $priid="";
my %options;
my $path;
my $infile="";
...
$share.="\/";
##check progroms
checkprogram($unafold);
checkprogram("$ct2out </dev/null >/dev/null");
##count default
$rstcount{'pri'}=0;
$rstcount{'area'}=0;
...
##################STEP1##################
step1:
@splitname=seqsplit_main($infile,500,150);
##################STEP2##################
step2:
callunafd('long');#call unafoldcel_candidate_mir_overview_20.out
##################STEP3##################
step3:
...
EOF
print 'Report bugs to Yonggan Wu (weasteam@gmail.com) or Dr. Simon Rayner (raynere@wh.iov.cn)', "\n";
    exit;
}
sub version ($) {
	print "\n";
    print "$_[0] (miRPare) $version\n";
...
sub seqsplit_main{#split sequence into small sequences
   #>>>>>>>>>>>>>>>>>>>>>>>>>>INTRODUCTION<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   #===========The seqsplit was used to split the given sequence into small ones
   #===========Usage: seqsplit($infile,splitlength,overlap)
   ...
   while (<IN>){
	  ...
	  if (index($_,">") ne "-1"){
		 if ($seq ne ""){
		 	$seq=lc($seq);
		 	$seq=~s/t/u/g;
			@tmpname=seqsplit($id,$seq,$slen,$overlap);
			push (@splitname,@tmpname);
		 }
		 $id=$_;
		 $id=~s/^>//;
		 if (index($id," ") ne "-1"){
...
sub seqsplit{#split sequence into small sequences
   #>>>>>>>>>>>>>>>>>>>>>>>>>>INTRODUCTION<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   #===========The seqsplit was used to split the given sequence into small ones
   #===========Usage: seqsplit($id,$seq,splitlength,overlap)
   #===========The $abname will be return to the main program.
   #===========The *_splitseq.nm will be created with the created seq (fasta format)
   #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>END<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   my ($id,$inseq,$slen,$overlap)=@_;#input the file
   my $splitseq;
   my $olseqs;#start
   my $olseqe;#end
   my $midseq;
...
sub callunafd{
    ...
    if ($parameter eq "long"){
    	open (IN,"$abname\_splitseq.fasta");
    		while (<IN>){
    			if ($_=~/^>/){
    				if ($id ne ""){
    					print "RNA folding with UNAFold to $id\n";
						system "$unafold tmp.fasta > error.log";
						if (-e "tmp.fasta_1.ct"){
...
sub readlongfile{
   ...
   foreach (@splitname){
	  ...
	  next if (not -e "$_\.str");
	  open (DATA,"$_\.str");#read the data of *.out file
	  @data=<DATA>;
	  close DATA;
	  system "rm $_\.str";
	  $do=0;
...
#	  $rstcount{'pri'}+=1;
	  ...
	  #unafold
	  system "$unafold tmp.fasta > error.log";
	  next if (not -e "tmp.fasta_1.ct");
	  system "$ct2out <tmp.fasta_1.ct> tmp.str";#run ct2out
	  system "rm tmp.fas*";
#	  callunafd("tmp");
	  next if (not -e "tmp.str");
...
sub out{
   ...
   if (exists($mirbase{lc($para{'miseq'})})){
	  $blast='('.$para{'miid'}.", Hits to miRBase13.0:".$mirbase{lc($para{'miseq'})}.')';
   }
   else{
	  $blast='('.$para{'miid'}.')';
   }
   $miseq=(" " x ($para{'mistart'}-1)).uc($para{'miseq'});
   open (OT,">>$abname\_mir_overview_$level.out");
   if ($priid eq "" or $priid ne $para{'priid'}){
   		print OT ">$para{'priid'}\n".uc($para{'priseq'})."\n";
   		$priid = $para{'priid'};
   }
   print OT $miseq.' '.$blast."\n";
   close OT;
   @pmt=pmt('all');
   $l=length($para{'miid'});
...
sub head{
   ...
   my $head; 
   my @data;
   $head.= "miRNA results predicted by miRPara$version\n";
   $head.= "By Yonggan Wu and Simon Rayner\n";
   $head.= 'Report bugs to Yonggan Wu (weasteam@gmail.com) or '."\n".'Dr. Simon Rayner (sraynere@wh.iov.cn)'."\n";
   $head.= 'Bioinformatics Group, State Key Laboratory of Virology'."\n";
   $head.= 'Wuhan Institute of Virology, Chinese Academy of Sciences.'."\n";
   $head.= "Wuhan, 430071, China\n\n";
   $head.= ("-" x 86)."\n";
   $head.="Your data was predicted using training data $species\n";
   $head.= "At level:           $level (Total 20 levels)\n";
...
#	  %para=();
	  ...
	  print "Predicting to $para{'miid'} at level $level.";
	  $tmp=$para{'miid'};
	  $tmp=~s/-//g;
...
sub checkprogram {
    my ($p)=@_;
    my $name=substr($p,0,index($p," "));
	($name)= ($name =~ m!([^/]+)$!);
    print "Checking for $name... ";
    if (eval { no warnings; open IN, "$p 2>&1 |" }) {
	  close IN;
	  print "found\n";
    }
	else {
	  print "not found\n";
	  die ("Please specify the installation of $p!\n");
    }
}
