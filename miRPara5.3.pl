...
if ($species=~/animal/ or $species=~/plant/ or $species=~/virus/){#to check the species
	$species=lc($species);
	my %spenames;
...
if ($onlypmt ne 1){
   #svm data
   $svmmodel = new Algorithm::SVM(Model => "$share/$species\_$level.model") or die ("Error: Could not load models, please type miRPara.pl -h for help!\n");
}
##################################start#########################################
